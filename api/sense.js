...
module.exports = async function handler(request, response) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    return sendJson(response, 500, { error: "Missing OPENAI_API_KEY on the server." });
  }
  ...
  if (!word) {
    return sendJson(response, 400, { error: "Word is required." });
  }
  ...
  try {
    ...
    return sendJson(response, 500, { error: "Could not generate a Word Sense answer." });
  }
};
